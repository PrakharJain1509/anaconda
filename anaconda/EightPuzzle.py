...
initial_state = []
print("Enter the initial state row by row (use space between numbers):")
for _ in range(3):
    row = input().split()
    initial_state.append([int(row[0]), int(row[1]), int(row[2])])  # Manually indexing

# Taking user input for goal state
goal_state = []
print("Enter the goal state row by row (use space between numbers):")
for _ in range(3):
    row = input().split()
    goal_state.append([int(row[0]), int(row[1]), int(row[2])])  # Manually indexing
